...
dim(trainA)
#only 35 rows, not a lot to work with
...
# Can I make a function for this so I can replicate it 100 times, 
...
tt_rep = function(data){
    ...
    test_train = bind_rows(val, real_train)
...
span_rmse <- function(span) {
    ...
    test_train <- tt_rep(trainA)
    
    test <- test_train[1:4,]
    train <- test_train[5:35,]
...
# 100 times and put the output into a tibble (I tried it using replicate before,
...
# let's plot the resulting data to try to find the span that corresponds to the
# lowest test_rmse
...
# Let's predict for testA using the loess model with span = 2
...
#of course RMSE is really low because it's overfitted
...
# Does not look like it's just noise!! Looks like an S curve. This will 
# obviously be better with loess than a linear model
...
# replicate the function 100 times for multiple spans
...
# hard to read but there seems to be a dip around 0.7
...
# Looks like it's around 0.75

# Let's predict for testB using the loess model with span = 0.75
...
splits_rmse_lm <- 
    replicate(1000, map_dfr(splits, ~train_split_lm(trainC, .x)), 
              simplify = TRUE)
...
# 10/90 is slightly lower, so I'll go with that. The linear model has lower
# average RMSEs, so I think we should go linear
